...
export default function AlertsManagementPage() {
  ...
  const handleIssueAlert = async (id: string) => {
    ...
    if (res.success) {
      fetchDraftAlerts();
      fetchAlerts(); // refresh main list too, since it's now "issued"
    }
  };

  const handleRejectAlert = async (id: string) => {
    if (!window.confirm('Reject this AI-detected warning? It will not be sent to citizens.')) return;
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="p-6 space-y-6">
            ...
            {(['all', 'active', 'inactive', 'pending'] as const).map((tab) => (
              ...
              >
                ...
                {tab === 'pending' && draftAlerts.length > 0 && (
                  <span className="absolute -top-1.5 -right-1.5 bg-purple-600 text-white text-[9px] font-bold rounded-full w-4 h-4 flex items-center justify-center">
                    {draftAlerts.length}
                  </span>
                )}
              </button>
        ...
        {filterActive === 'pending' ? (
          draftLoading ? (
            <LoadingState message="Loading pending reviews..." />
          ) : draftAlerts.length === 0 ? (
            <EmptyState
              icon={<CheckCircle2 className="w-10 h-10 text-slate-300" />}
              title="No pending reviews"
              message="AI hasn't flagged any new risk warnings awaiting your approval."
            />
          ) : (
            <div className="space-y-3">
              {draftAlerts.map((alert) => (
                <div
                  key={alert._id}
                  className="p-5 bg-white rounded-xl border border-purple-200 bg-purple-50/20 shadow-xs"
                >
                  <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3">
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                      <button
                        ...
                        className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold text-emerald-700 bg-emerald-50 hover:bg-emerald-100 rounded-lg transition-colors border border-emerald-200 active:scale-95"
                      >
