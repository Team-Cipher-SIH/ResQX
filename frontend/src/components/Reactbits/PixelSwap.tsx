...
> = {
  ...
  spiral: (x, y) => {
    const angle =
      (Math.atan2(y - 0.5, x - 0.5) + Math.PI) /
      (Math.PI * 2);
...
const buildGrid = ({
  ...
  let columns = Math.max(
    1,
    Math.ceil((width + gap) / (size + gap))
  );

  let rows = Math.max(
    1,
    Math.ceil((height + gap) / (size + gap))
  );
  ...
  const originX =
    (width - (columns * stride - gap)) / 2;

  const originY =
    (height - (rows * stride - gap)) / 2;

  const order =
    PATTERNS[pattern] ?? PATTERNS.random;
...
function PixelSwap({
  ...
  const [shownActive, setShownActive] =
    useState(
  ...
  useEffect(() => {
    ...
    frozenGrid.pixels.forEach(
      (pixel, index) => {
        const pixelElement =
          pixelRefs.current[index];
